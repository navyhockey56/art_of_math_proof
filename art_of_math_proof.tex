...
Let's be honest, no one wants to spend the time to write out every proof in as much detail as we have within this article. Especially considering that our first attempt at a proof is bound to be riddled with things that will eventually get crossed out. To help ease the tedium, mathematicians have developed several notations to make writing proofs faster. As such, I have included several of the most common notations below. I must warn you that not every mathematician ascribes the same meaning to each of these notations, so be careful in your assumptions when presented with a new author.\\
\\
...
\textbf{Meaning}: "then"\\
\textbf{Example with symbol}: If $n$ and $m$ are odd $\rightarrow$ their sum is odd.\\
\textbf{Example without symbol}: If $n$ and $m$ are odd then their sum is odd.\\
\\
