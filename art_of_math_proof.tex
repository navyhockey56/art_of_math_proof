...
$s = 2*(j + k) = 2*p$.\\ 
\\
Therefor, since $p$ is an integer, and $s$ is equal to $2*p$, $s$ is even by definition.\\ 
\\
Furthermore, $s$ is not only even, it's also the sum of $n$ and $m$ - which were represented in such a way that our argument could apply to any pair of even integers, thus we have completed our proof! \\
\\
